...
def init_sim_data(sim_params, ocp_params, x0):
    ...
    sim_data['nbody'] = ocp_params['nbody']
    ...
    sim_data['pose_pred'] = np.zeros((sim_data['N_mpc'], ocp_params['N_h']+1, 7))                   # Predicted EE pose (Cartesian pose , Quaternions)
    ...
    # Measurements
    sim_data['state_mea_SIM_RATE'] = np.zeros((sim_data['N_sim']+1, sim_data['nx']))            # Measured states ( x^mea = (q, v) from actuator & Mujoco at SIMU freq )
    sim_data['force_mea_SIM_RATE'] = np.zeros((sim_data['N_sim'], 6)) 
    sim_data['force_q_app_SIM_RATE'] = np.zeros((sim_data['N_sim']+1, sim_data['nq']))            # Applied generalized force in Mujoco to the robot
    sim_data['force_x_app_SIM_RATE'] = np.zeros((sim_data['N_sim']+1, sim_data['nbody'], 6))                         # Applied Cartesian force in Mujoco to the robot
    sim_data['state_mea_SIM_RATE'][0, :] = x0
    # IRL-related data
    sim_data['IRL_state'] = []
    sim_data['IRL_ctrl'] = []
    for i in range(sim_data['N_h']-1):
        sim_data['IRL_state'].append(np.zeros((i+1, sim_data['nx'])))
        sim_data['IRL_ctrl'].append(np.zeros((i+1, sim_data['nu'])))
    sim_data['IRL_state'].append(np.zeros((sim_data['N_h'], sim_data['nx'])))
